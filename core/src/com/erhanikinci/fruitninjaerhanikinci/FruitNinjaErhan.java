...
public class FruitNinjaErhan extends ApplicationAdapter implements InputProcessor {
	SpriteBatch batch;
	Texture backgroung;
	Texture apple;
	Texture bill;
	...
	public void create () {
		...
		backgroung = new Texture("ninjabackground.png");
	...
	public void render () {
		...
		if (lives > 0){
			...
			for (int i = 0; i < lives; i++){
				batch.draw(apple, i * 60f + 40f, Gdx.graphics.getHeight()-70f, 50f, 50f);
			}
			...
			for (Fruit fruit :fruitArray){
				if (fruit.outOfScreen()){
					...
					//ekranın altına düşen bir elma ise lives' i bir azaltmam gerekiyor.
		...
		font.draw(batch, "Score: " + score, 40, 80);
		if (lives <= 0){
			font.draw(batch, "Cut to play", Gdx.graphics.getWidth() * 0.4f, Gdx.graphics.getHeight() * 0.5f);
		}
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		if (lives <= 0 && currentTime - gameOverTime > 2f){
			...
			for (Fruit f : fruitArray){
				...
				System.out.println("distance: " + f.clicked(f.pos));
